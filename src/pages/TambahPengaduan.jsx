...
import { tambahKasus } from "../api/kasusServices";
...
export default function TambahPengaduan() {
  ...
  const handleTambah = async () => {
    setLoading(true);
    try {
      const data = await tambahKasus();
      console.log("📦 RESPONSE DARI BACKEND:", data);

      if (data?.kasus_id) {
        // ✅ Simpan nomor_registrasi ke formData agar tampil
        
        
        // alert("Kasus baru berhasil dibuat!");
        alert(`Kasus baru berhasil dibuat!\nNomor Registrasi: ${data.nomor_registrasi}`);

        navigate(`/dashboard/pengaduan/${data.kasus_id}/data-diri`);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="bg-white shadow-lg rounded-lg p-6 max-w-md mx-auto text-center">
        ...
        <p className="text-2s font-semibold text-gray-600 mb-6">
        ...
        <div className="space-y-3 text-left">
          {/* <h2 className="text-lg font-semibold text-center">Awal Pendaftaran</h2> */}
        ...
        <button
          onClick={() => navigate("/dashboard/kasus")}
